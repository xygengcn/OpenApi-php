...
class Bt
{
     ...
     * @param [type] $sitename  网站名
     * @param [type] $subfilter 文本替换json格式
     */
    ...
    {
        $url = $this->BT_PANEL . $this->config('crontabList');
        $p_data = $this->GetKeyData();
        $p_data['name'] = '';
        $p_data['sType'] = '';
        $result = $this->HttpPostCookie($url, $p_data);
    ...
    {
        $url = $this->BT_PANEL . $this->config('AddCrontab');
    ...
    {
        //定义cookie保存位置
        $cookie_file = 'app/cookie/' . md5($this->BT_PANEL) . '.cookie';
        if (!file_exists($cookie_file)) {
            $fopen = fopen($cookie_file, 'wb '); //新建文bai件命令du
            fputs($fopen, ''); //向文zhi件中写入内容;
            fclose($fopen);
        }
    ...
    {
        require_once(__ROOT__ . '/app/config/BtConfig.php');
        return $config[$str];
    }
}